...
async function handleFormSubmit(formData: FormData) {
  ...
  // Perform server-side logic here (Database query, Auth API, session cookie, etc.)
  console.log("Server processing auth:", { mode, name, email, password, rememberMe });
...
async function handleGoogleSignIn() {
  ...
  // Perform OAuth redirection logic here
...
export default async function Auth({ searchParams }: AuthProps) {
  ...
  const showPassword = params.showPassword === "true";
  ...
  return (
    <main className="min-h-dvh flex flex-col justify-center items-center bg-slate-50 text-slate-800 p-4 md:p-6 relative font-sans">
      ...
      <div className="w-full max-w-md bg-white rounded-3xl border border-slate-200/80 shadow-xl overflow-hidden my-12">
        ...
        {/* Auth Mode Toggle Tabs (Server Navigation via Query Parameters) */}
        <div className="flex border-b border-slate-100 bg-slate-50/50 p-1.5">
          <Link
            href="/auth"
            className={`flex-1 py-3 text-center text-xs md:text-sm font-bold uppercase tracking-wider rounded-xl transition-all ${
              !isSignUp 
    ...
    >
            <button
              type="submit"
              className="w-full py-3.5 px-4 rounded-xl border border-slate-200 hover:border-slate-300 bg-white hover:bg-slate-50 text-slate-700 font-semibold text-sm flex items-center justify-center gap-3 transition-all shadow-sm active:scale-[0.99]"
            >
          ...
          <form action={handleFormSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                {/* Toggle Password Visibility using URL parameters */}
                <Link
                  href={
                    showPassword
                      ? `/auth${isSignUp ? "?mode=signup" : ""}`
                      : `/auth?${isSignUp ? "mode=signup&" : ""}showPassword=true`
                  }
                  className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 text-sm flex items-center justify-center"
                  replace
                >
            ...
            <button
              type="submit"
              className="w-full mt-2 py-4 rounded-xl bg-[#E73F1E] hover:bg-orange-700 text-white font-bold text-xs md:text-sm uppercase tracking-wider transition shadow-lg active:scale-[0.99] flex items-center justify-center gap-2"
            >
          ...
          <p className="text-center text-xs text-slate-500 pt-2">
            {isSignUp ? (
              ...
              <>
                ...
                <Link href="/auth?mode=signup" className="font-bold text-[#E73F1E] hover:underline">
                  Sign Up for Free
                </Link>
              </>
            )}
