    ...
    return(
        <main className="flex items-center justify-between lg:px-20 py-2 shadow-md relative z-50">
            ...
            <Image
                ...
                className="w-15 h-15"
                />
                <p className="text-xl italic text-gray-700">Kravings</p>
            </Link>
            ...
            <div className="flex items-center gap-6 ml-6 max-lg:hidden">
                <Link href={"/auth"} className="flex items-center border gap-1 px-6 py-1 rounded-full
                border-gray-500 text-lg">Account<FaCircleUser /></Link>
                <Link href={"/"} style={{backgroundColor: Theme.primaryColor, borderColor:Theme.primaryColor}} className="px-6 py-1
                rounded-full text-white border text-lg">Add Recipe</Link>
            </div>
            {/*mobile and tablet view */}
            <button onClick={()=> setNavOpen(!navOpen)} className="lg:hidden z-50 text-3xl mr-2">
                {
                    navOpen ?   <IoMdRestaurant /> :  <RiMenuAddFill />
                }
            </button>

            <blockquote className={`lg:hidden absolute top-0 right-0 w-full h-dvh space-y-6 bg-white ${navOpen ?
                "block" : "hidden"}`}>
            ...
            <div className="flex flex-col items-center gap-6">
                <Link href={"/"} className="flex items-center border gap-1 px-6 py-1 rounded-full
                border-gray-500 text-lg">Account<FaCircleUser /></Link>
                <Link href={"/"} style={{backgroundColor: Theme.primaryColor, borderColor:Theme.primaryColor}} className="px-6 py-1
                rounded-full text-white border text-lg">Add Recipe</Link>
            </div>
