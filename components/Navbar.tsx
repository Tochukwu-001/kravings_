...
export default function Navbar() {
    ...
    return (
        <main className="flex items-center justify-between lg:px-20 py-2 shadow-md relative z-50">
            ...
            <div className="flex items-center gap-6 ml-6 max-md:hidden">
                <Link href={"/auth"} className="flex items-center border gap-1 px-6 py-1 rounded-full border-gray-700 text-lg">
            ...
            <blockquote className={`lg:hidden absolute top-0 right-0 w-full h-dvh space-y-6 bg-white ${navOpen ? 'block' : 'hidden'}`}>
                ...
                <div className="flex flex-col items-center gap-6">
                    <Link href={"/"} className="flex items-center border gap-1 px-6 py-1 rounded-full border-gray-700 text-lg">
                        Account <FaRegCircleUser />
                    </Link>
                    <Link href={"/"} style={{ backgroundColor: Theme.primaryColor, borderColor: Theme.primaryColor }}
                        className="px-6 py-1 rounded-full text-white border text-lg">Add Recipe</Link>
